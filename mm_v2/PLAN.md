...
- **REST top-level taker-fill capture (Contract #1b additive; M0-verify).** A `/data/trades` row can carry the user's fill on the top-level row with `maker_orders: []` and `trader_side: "TAKER"`. `RestTradeEvent` now preserves `trader_side` (parser_version bumped to 2), and `derive_logical_fills` synthesizes one top-level `LogicalFill` (keyed by `taker_order_id`) only for a confirmed `TAKER`; an empty-`maker_orders` row that is not a confirmed TAKER raises `UnattributableRestFillError` (loud, never a silent drop). The reconciler (WS-D) MUST catch this **per row** — emit a marker and continue the poll. Dedup invariant: the user is maker XOR taker per trade, so taker-keyed and maker-keyed fills never both represent the same fill. M0 must verify that `TAKER` + empty `maker_orders` reliably means the user's own taker fill, and that populated `maker_orders` carries the user's fills.
